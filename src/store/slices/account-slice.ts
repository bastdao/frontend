...
export const getBalances = createAsyncThunk("account/getBalances", async ({ address, networkID, provider }: IGetBalances): Promise<IAccountBalances> => {
    ...
    const memoContract = new ethers.Contract(addresses.HIHIDAO_ADDRESS, MemoTokenContract, provider);
    const memoBalance = await memoContract.balanceOf(address);
    const timeContract = new ethers.Contract(addresses.HOHODAO_ADDRESS, TimeTokenContract, provider);
...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.HIHIDAO_ADDRESS) {
        ...
        if (addresses.WHIHIDAO_ADDRESS) {
            memoWmemoAllowance = await memoContract.allowance(address, addresses.WHIHIDAO_ADDRESS);
        }
    }
