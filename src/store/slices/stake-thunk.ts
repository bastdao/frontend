...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const timeContract = new ethers.Contract(addresses.HOHODAO_ADDRESS, TimeTokenContract, signer);
    const memoContract = new ethers.Contract(addresses.HIHIDAO_ADDRESS, MemoTokenContract, signer);
