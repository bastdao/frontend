...
export const changeApproval = createAsyncThunk("wraping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const memoContract = new ethers.Contract(addresses.HIHIDAO_ADDRESS, wMemoTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await memoContract.approve(addresses.WHIHIDAO_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wmemoAllowance = await memoContract.allowance(address, addresses.WHIHIDAO_ADDRESS);
...
export const changeWrap = createAsyncThunk("wraping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wmemoContract = new ethers.Contract(addresses.WHIHIDAO_ADDRESS, wMemoTokenContract, signer);
...
export const calcWrapDetails = createAsyncThunk("wraping/calcWrapDetails", async ({ isWrap, value, provider, networkID }: IWrapDetails, { dispatch }) => {
    ...
    const wmemoContract = new ethers.Contract(addresses.WHIHIDAO_ADDRESS, wMemoTokenContract, provider);

    if (isWrap) {
        const wmemoValue = await wmemoContract.HIHIDAOTowHIHIDAO(amountInWei);
        wrapValue = wmemoValue / Math.pow(10, 18);
    } else {
        const memoValue = await wmemoContract.wHIHIDAOToHIHIDAO(amountInWei);
